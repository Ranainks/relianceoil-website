...
const markdownComponents = {
  h2: ({ children }) => <h2 style={{ fontSize: 'clamp(1.15rem, 4vw, 1.3rem)', fontWeight: 800, color: '#111', marginTop: '2rem', marginBottom: '0.75rem' }}>{children}</h2>,
  ...
  img: ({ src, alt }) => <img src={src} alt={alt} style={{ width: '100%', maxWidth: '100%', borderRadius: '12px', margin: '1.5rem 0', objectFit: 'cover' }} />,
  ...
  if (notFound) {
    return (
      <div style={{ minHeight: '60vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '16px' }}>
        ...
        <Link to="/news" style={{ color: '#CC0000', fontWeight: 600, display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '6px' }}>
      ...
      <div style={{ background: categoryColors[post.category] || categoryColors['Company News'], padding: '60px 24px 80px' }}>
        <div style={{ maxWidth: '800px', width: '100%', margin: '0 auto' }}>
          <button
            ...
            style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '6px', color: 'rgba(255,255,255,0.8)', background: 'none', border: 'none', cursor: 'pointer', fontSize: '0.875rem', marginBottom: '24px', padding: 0 }}
          >
            <FaArrowLeft size={12} /> Back to News
          </button>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '20px', color: 'rgba(255,255,255,0.75)', fontSize: '0.85rem' }}>
            <span style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '6px' }}><FaCalendarAlt size={12} />{post.date}</span>
            <span style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '6px' }}><FaUser size={12} />{post.author}</span>
            {post.video_url && <span style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '6px' }}><FaPlayCircle size={12} /> Video included</span>}
          </div>
        </div>
      </div>

      <div style={{ maxWidth: '800px', width: '100%', margin: '0 auto', padding: '48px 24px' }}>
        {post.image_url && !youtubeId && (
          <img
            src={post.image_url}
            alt={post.title}
            style={{ width: '100%', maxWidth: '100%', borderRadius: '16px', marginBottom: '40px', objectFit: 'cover', maxHeight: '420px' }}
          />
        )}
        ...
        {post.video_url && !youtubeId && (
          <video
            controls
            style={{ width: '100%', maxWidth: '100%', borderRadius: '16px', marginBottom: '40px' }}
          >
            <source src={post.video_url} />
          </video>
        )}

        <p style={{ fontSize: 'clamp(1rem, 3.5vw, 1.1rem)', color: '#444', lineHeight: 1.8, fontWeight: 500, marginBottom: '32px', borderLeft: '4px solid #CC0000', paddingLeft: '20px', overflowWrap: 'break-word' }}>
          {post.excerpt}
        </p>
