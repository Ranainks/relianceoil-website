...
export default function EidGreeting() {
  ...
  useEffect(() => {
    ...
    if (sessionStorage.getItem('eid_adha_v3')) return;
    const t = setTimeout(() => {
  ...
  function close() {
    sessionStorage.setItem('eid_adha_v3', '1');
    setVisible(false);
  }
    ...
    <AnimatePresence>
      {visible && (
        ...
        >
          ...
          >
            {/* ── HEADER ── */}
            <div style={{ background: 'linear-gradient(135deg,#7f0000 0%,#CC0000 55%,#990000 100%)', padding: '36px 32px 28px', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
              <div style={{ position:'absolute', top:'-50px', left:'-50px', width:'200px', height:'200px', borderRadius:'50%', border:'1px solid rgba(255,255,255,0.08)', pointerEvents:'none' }} />
              <div style={{ position:'absolute', bottom:'-30px', right:'-40px', width:'160px', height:'160px', borderRadius:'50%', border:'1px solid rgba(255,215,0,0.12)', pointerEvents:'none' }} />
              ...
              <div style={{ display:'inline-block', marginBottom:'14px', position:'relative', zIndex:1 }}>
                <svg viewBox="0 0 64 64" width="64" height="64">
                  ...
                  <circle cx="46" cy="23" r="23" fill="#990000" />
                  <circle cx="54" cy="28" r="4.5" fill="#FFD700" />
                </svg>
              </div>

              <div style={{ fontSize:'0.67rem', fontWeight:700, color:'rgba(255,255,255,0.65)', textTransform:'uppercase', letterSpacing:'0.14em', marginBottom:'8px', position:'relative', zIndex:1 }}>
                28 May 2026
              </div>
              <h2 style={{ fontWeight:900, fontSize:'1.85rem', color:'#FFD700', lineHeight:1.1, margin:'0 0 8px', textShadow:'0 2px 14px rgba(0,0,0,0.4)', position:'relative', zIndex:1 }}>
                Eid al-Adha Mubarak
              </h2>
              <p style={{ color:'rgba(255,255,255,0.75)', fontSize:'1.15rem', margin:0, fontFamily:'serif', letterSpacing:'0.06em', position:'relative', zIndex:1 }}>
                عيد الأضحى مبارك
              </p>
            </div>

            {/* ── SHEEP STRIP ── */}
            <div style={{ backgroundColor:'#fff5f5', height:'108px', position:'relative', overflow:'hidden', borderTop:'2px solid rgba(204,0,0,0.1)', borderBottom:'2px solid rgba(204,0,0,0.1)' }}>
              <div style={{ position:'absolute', bottom:0, left:0, right:0, height:'16px', background:'linear-gradient(to right,#ffc9c9,#ffaaaa,#ffc9c9)', borderRadius:'60% 60% 0 0 / 100% 100% 0 0' }} />
              <WalkingSheep />
            </div>
            ...
            <div style={{ backgroundColor:'#fff', padding:'24px 32px 28px', textAlign:'center' }}>
              <p style={{ color:'#333', fontSize:'0.95rem', lineHeight:1.8, margin:'0 0 6px' }}>
                <strong style={{ color:'#CC0000' }}>Reliance Oil Limited</strong> wishes you and your family a blessed Eid filled with peace, joy, and togetherness.
              </p>
              ...
              <button
                onClick={close}
                style={{ backgroundColor:'#CC0000', color:'#fff', border:'none', borderRadius:'9999px', padding:'13px 36px', fontWeight:700, fontSize:'0.9rem', cursor:'pointer', width:'100%', letterSpacing:'0.03em', transition:'background-color 0.2s' }}
